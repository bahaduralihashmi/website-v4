...
import { MessageSquare, Compass, Eye, ShoppingCart } from "lucide-react";
...
export default function TireCard({ tire, darkMode, onSelect }: TireCardProps) {
  ...
  const isAutogrip = String(tire.brand || "").toLowerCase() === "autogrip";

  const addToCart = () => {
    window.dispatchEvent(new CustomEvent("hbt-add-to-cart", { detail: { tire } }));
  };
    ...
    <motion.div layout initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} whileHover={{ y: -6, transition: { duration: 0.2 } }} transition={{ duration: 0.3 }} className={`relative rounded-3xl overflow-hidden flex flex-col justify-between border transition-all duration-300 group ${darkMode ? "bg-[#1c1c24] border-white/5 hover:border-brand-orange/40 shadow-xl shadow-black/20" : "bg-white border-slate-100/90 hover:border-brand-orange/40 shadow-md shadow-slate-100"}`}>
      ...
      <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
        <div>
          ...
          <div className={`mt-3 flex items-center space-x-1.5 text-xs font-semibold ${darkMode ? "text-slate-300" : "text-slate-700"}`}><Compass className="w-3.5 h-3.5 text-brand-orange shrink-0" /><span className="truncate">{tire.feature}</span></div>
        </div>
        <div className="pt-4 border-t border-black/5 dark:border-white/5">
          <div className="flex items-baseline justify-between mb-4 gap-3"><span className={`text-[10px] uppercase font-mono tracking-wider ${darkMode ? "text-slate-400" : "text-slate-500"}`}>Listed price</span><span className="text-brand-orange font-display text-xl font-extrabold tracking-tight">PKR {tire.price.toLocaleString()}</span></div>
          <div className="grid grid-cols-5 gap-2">
            <button onClick={() => onSelect(tire)} className={`col-span-1 flex items-center justify-center p-3 rounded-xl transition-all ${darkMode ? "bg-white/5 hover:bg-white/10 border border-white/5 text-slate-300 hover:text-white" : "bg-slate-100 hover:bg-slate-200 border border-slate-200/50 text-slate-700 hover:text-slate-950"}`} title="View tyre specifications" aria-label={`View specifications for ${tire.name}`}><Eye className="w-4.5 h-4.5" /></button>
            <button onClick={addToCart} disabled={tire.stock === 0} className="col-span-2 flex items-center justify-center gap-1.5 bg-brand-orange hover:bg-orange-500 disabled:opacity-40 disabled:cursor-not-allowed text-white py-3 px-2 rounded-xl text-xs font-bold transition-all" aria-label={`Add ${tire.name} to cart`}><ShoppingCart className="w-4 h-4" /><span>Add to Cart</span></button>
            <a href={waLink} target="_blank" rel="noopener noreferrer" className="col-span-2 flex items-center justify-center space-x-1.5 bg-green-600 hover:bg-green-500 text-white py-3 px-2 rounded-xl text-xs font-bold transition-all hover:shadow-md hover:shadow-green-500/10" aria-label={`Ask HBT about ${tire.name}`}><MessageSquare className="w-4 h-4 shrink-0" /><span>WhatsApp</span></a>
          </div>
        </div>
      </div>
    </motion.div>
